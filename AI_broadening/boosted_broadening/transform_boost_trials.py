...
def log_cosh_loss(y_true, y_pred):
	"""Compute log-cosh loss and its gradient for XGBoost."""
	error = y_pred - y_true
	grad = np.tanh(error)  # First derivative (gradient)
	hess = 1 / np.cosh(error) ** 2  # Second derivative (Hessian)
	return grad, hess
...
plt.legend()
plt.title(f'{periodictable.elements[nuclide[0]]}-{nuclide[1]} $\sigma_{{n,\gamma}}$ at {test_temperatures[0]} K')
plt.savefig(f'/home/rnt26/PycharmProjects/ResonanceML/AI_broadening/boosted_broadening/transformtrialplots/{get_datetime_string()}-transformplot.png')
plt.yscale('log')
plt.xscale('log')
plt.show()
